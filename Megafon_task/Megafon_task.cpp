...
vector<vector<bool>> field2(n, vector<bool>(n, 0)); // создаем два массива , в одном будем хранить текущие значения ячеек , во вторую запишем результаты вычислений 
int mod(int a, int b)
{
    int c;
    if (a >= 0)
...
int count_life(int x, int y)
{
    ...
    for (int i = x - 1; i < x + 2; i++)
    {
     ...
     // в данном цикле  рассчитываем новые значения для каждой клетки , в частности , для крайних клеток где как раз используется написанная выше функция mod
