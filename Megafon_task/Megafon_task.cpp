...
vector<vector<bool>> field2(n, vector<bool>(n, 0)); // создаем два массива , в одном будем хранить текуцщие значения ячеек , во вторую запишем результаты вычислений 
int mod(int a, int b)
{
    int c;
    if (a >= 0)
...
//это понадобится в дальнейшем для вычисления значений ,а конкрентно для замыкания поля , т.е мы соединяем левый край с правым , верхний край с нижним .
int count_life(int x, int y)
{
    ...
    for (int i = x - 1; i < x + 2; i++)
    {
     ...
     // в данном цикле мы рассчитываем новуе значения для каждой клетки , в частности для крайних клеток где как раз используется написанная выше функция mod
...
}// в данной функции заполняем массив случайными значениями 
void print()
{
    for (int i = 0; i < n; i++)
    {
